...
d3.csv("data/sensors_daily_features.csv", function (error, data) {
    ...
    svg.selectAll("rect")
       ...
       .attr({
           x: function (d, i) { return i * (width / s.length); },
            ...
            height: height,
            fill: function (d) { return colorPicker(d.zone_MDS); }
       })
   ...
   .attr({
       ...
       width: width / s.length - padding,
       height: height,
       fill: function (d) { return colorPicker(d.zone_Growth); }
   })
 .on("mouseover", function (d) {
     tooltip.transition()
       .duration(500)
       .style("opacity", .9);
     tooltip.html("<strong>Date: " + d.thedate + "<br/>Zone: " + d.zone_Growth + "</strong>")
        .style("left", (d3.event.pageX) + "px")
        .style("top", (d3.event.pageY - 28) + "px");
 })
  .on("mouseout", function (d) {
 ...
 .on("mouseover", function (d) {
     tooltip.transition()
       .duration(500)
       .style("opacity", .9);
     tooltip.html("<strong>Date: " + d.thedate + "<br/>Zone: " + d.Grade + "</strong>")
        .style("left", (d3.event.pageX) + "px")
        .style("top", (d3.event.pageY - 28) + "px");
 })
  .on("mouseout", function (d) {
