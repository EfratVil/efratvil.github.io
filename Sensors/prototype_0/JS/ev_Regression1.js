...
d3.csv("data/sensors_daily_features.csv", function (error, data) {
    ...
    var sel = d3.select('#x_asset').node().value;
    var data1 = data.filter(function (d) { return d.thedate == sel && d.Include_Flag == "Y" && isNaN(d.Growth) === false });

    x.domain(d3.extent(data1, function (d) { return d[x_asset]; })).nice();
    y.domain([d3.min(data1, function (d) { return d[y_asset]; }), d3.max(data1, function (d) { return d[y_asset]; })]).nice();
    ...
    .on("mouseover", function (d) {
        tooltip.transition()
            .duration(500)
            .style("opacity", .9);
        tooltip.html("Project: " + d.project_id + "<br/>" + "Sensor: " + d.sensor_id + "<br/>" + "MDS: " + d[x_asset] + "<br/>Growth: " + d[y_asset])
    ...
    var xSeries = data1.map(function (d) { return parseFloat(d[x_asset]); });
    var ySeries = data1.map(function (d) { return parseFloat(d[y_asset]); });
    ...
    var x1 = d3.min(data1, function (d) { return d[x_asset]; });
    var y1 = leastSquaresCoeff[0] * d3.min(data1, function (d) { return d[x_asset]; }) + leastSquaresCoeff[1];
    var x2 = d3.max(data1, function (d) { return d[x_asset]; });
    var y2 = leastSquaresCoeff[0] * d3.max(data1, function (d) { return d[x_asset]; }) + leastSquaresCoeff[1];
    ...
    function ChangeAsset() {
        ...
        sel = d3.select('#x_asset').node().value;
        data1 = data.filter(function (d) { return d.thedate == sel && d.Include_Flag == "Y" && isNaN(d.Growth) === false });
    ...
    //    console.log("y_asset - " + y_asset);
     ...
     // .style("fill", function (d) { return colorPicker(3); });
        ...
        xSeries = data1.map(function (d) { return parseFloat(d[x_asset]); });
        ySeries = data1.map(function (d) { return parseFloat(d[y_asset]); });

        leastSquaresCoeff = leastSquares(xSeries, ySeries);

        x1 = d3.min(data1, function (d) { return d[x_asset]; });
        y1 = leastSquaresCoeff[0] * d3.min(data1, function (d) { return d[x_asset]; }) + leastSquaresCoeff[1];
        x2 = d3.max(data1, function (d) { return d[x_asset]; });
        y2 = leastSquaresCoeff[0] * d3.max(data1, function (d) { return d[x_asset]; }) + leastSquaresCoeff[1];
