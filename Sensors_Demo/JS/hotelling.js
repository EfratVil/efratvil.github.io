...
//================= Read file & create SVG =====================
  d3.csv("data/outliers_Week9.csv", function (error, data) {
    ...
    d3.select("#x_asset_lbl").html("Sensor Index");
    d3.select("#y_asset_lbl").html("Grade");

    var sel = "Week9" //d3.select('#week').node().value;
    ...
    data.forEach(function (d,i) {
        ...
        //grade_auto,grade,T2_basic,T2_MVE,T2_MCD,T2_UCL
    });

    var ucl = data[0]["T2_UCL"];

    var data1 = data.filter(function (d) { return isNaN(d.corr_sum) === false });
    console.log(data1);
    x.domain(d3.extent(data1, function (d) { return d.index; })).nice();
    y.domain([d3.min(data1, function (d) { return d["T2_basic"]; }), d3.max(data1, function (d) { return d["T2_basic"]; })]).nice();
    ...
    svg.selectAll(".dot")
      ...
      .enter().append("circle")
        ...
        .attr("cy", function (d) { return y(d["T2_basic"]); })
        .attr("opacity", 0.5)
            ...
            case "PC10":
                ...
                svg.selectAll('circle')
                   ...
                   .style("fill", function (d) {
                       if (d.nds_wt_PC_dist_10 > 3)
                   { return colorPicker_grade("Outlier"); }
                                                 else
                   { return colorPicker_grade("Good"); }
                   });
                break;
            //case "DN_T2_Hourly":
    ...
    function Update_chart() {
        ...
        var corr_y = d3.select('#y').node().value;
       // var corr_x = d3.select('#y').node().value;
        ...
        data.forEach(function (d,i) {
            ...
            d["index"] = i;
            d["corr_sum"] = +d["index"];  ///corr_y
            d["corr_sum_square"] = +d[corr_y]; //corr_x
        });
