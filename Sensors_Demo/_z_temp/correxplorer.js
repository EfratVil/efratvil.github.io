...
d3.csv("data/res_cor.csv", function (data) {
    var label_col_full = Object.keys(data[0]);
    label_col_full.pop();
    //console.log(label_col_full);
      var label_row = [];
      var rows = [];
      var row = [];
      ...
      label_row = label_col_full;
      main(rows, label_col_full, label_row);
...
var main = function(corr, label_col, label_row){
  ...
  var row = corr;
  var col = corr // for corr matrix, no need to transpose. d3.transpose(corr);
  console.log(row);
  console.log(col);
  ...
  var scale = d3.scale.linear()
      ...
      .range([0, parseFloat(0.7) * 750]); //d3.select("input#zoom")[0][0].value            !!!!!!!!!!!!!!!!!!!!! size
